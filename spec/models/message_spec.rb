...
RSpec.describe Message, type: :model do
  ...
  describe 'create message' do
    ...
    context 'with valid attributes' do
      let(:user) { create(:room_participant, room:) }
      let(:message) { build(:message, user: user.user, room:) }
    ...
    context 'when user blocked' do
      ...
      let(:message) { build(:message, user: participant.user, room:) }
    ...
    context 'when user is not participant' do
      ...
      let(:message) { build(:message, user: new_user, room:) }
      before { message.valid? }
    ...
    context 'when room is closed' do
      let(:message) { build(:message, room:) }
